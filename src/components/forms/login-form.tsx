...
export function LoginForm({ googleEnabled }: { googleEnabled: boolean }) {
  ...
  const { loading, error, fieldErrors, submit } = useApiForm('/api/auth/login', {
    resetForm: false,
    onSuccess: (data) => {
      const result = data as { otpRequired?: boolean; identifier?: string } | undefined;
      // When the SUPER_ADMIN has OTP enabled globally, login is a two-step
      // flow and the session is only created after the code is verified.
      if (result?.otpRequired) {
        router.push(
          `/verify-otp?identifier=${encodeURIComponent(result.identifier ?? '')}&purpose=LOGIN&next=${encodeURIComponent(next)}`,
        );
        return;
      }
      router.push(next);
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Sign in to manage your bookings, payments and requests.
      </p>
